...
am <- do.call(rbind,sapply(1:5,function(simuround) {
	do.call(rbind,sapply(c(0.25,0.5,0.75),function(per) {
		tmpd <- d
		tmpd$gene <- sapply(d$gene,function(i) {
			gl <- strsplit(i,', ')[[1]]
			snum <- floor(length(gl)*per)
			paste0(c(sample(gl,snum)),collapse = ',')
		})
		data.frame(simuround=simuround,noise=per,celltype=tmpd$celltype,gene=tmpd$gene)
	},simplify = F))
},simplify = F))
write.csv(am,'Dropbox/research/gptcelltype/simu/gpt4aug3/subset.csv',row.names = F)
