bcl <- table(readRDS('/home/zji4/data-hji7/zji/celltype/cancer/bcl/data/proc/ct.rds'))
bcl <- data.frame(dataset='BCL',tissue='B cell lymphoma',celltype=names(bcl),number=as.vector(bcl),stringsAsFactors = F)
coloncancer <- table(readRDS('/home/zji4/data-hji7/zji/celltype/cancer/coloncancer/data/proc/ct.rds'))
...
lungcancer <- table(readRDS('/home/zji4/data-hji7/zji/celltype/cancer/lungcancer/data/proc/ct.rds'))
...
hcaf <- list.files('/home/zji4/data-hji7/zji/celltype/hca/data/proc/')
hca <- sapply(hcaf,function(sf) {
	table(readRDS(paste0('/home/zji4/data-hji7/zji/celltype/hca/data/proc/',sf,'/ct.rds')))
},simplify = F)
...
hcl <- table(readRDS('/home/zji4/data-hji7/zji/celltype/hcl/data/proc/anno.rds')$celltype)
hcl <- data.frame(dataset='HCL',tissue=NA,celltype=names(hcl),number=as.vector(hcl),stringsAsFactors = F)

mca <- table(readRDS('/home/zji4/data-hji7/zji/celltype/mca/data/proc/anno.rds')$celltype)
mca <- data.frame(dataset='MCA',tissue=NA,celltype=names(mca),number=as.vector(mca),stringsAsFactors = F)

tsf <- list.files('/home/zji4/data-hji7/zji/celltype/tabulasapiens/data/meta/')
ts <- sapply(tsf,function(sf) {
	table(readRDS(paste0('/home/zji4/data-hji7/zji/celltype/tabulasapiens/data/meta/',sf))$cell_ontology_class)
},simplify = F)
...
saveRDS(d,file='/home/zji4/data-hji7/zji/celltype/numcell/numcell.rds')
