...
for (f in af) {
	a <- readRDS(paste0('/celltype/tabulasapiens/data/meta/',f))
	...
	rt <- system.time({
		...
		clu <- a[match(colnames(es.max),a[,1]),'cell_ontology_class']
		ss <- rowsum(t(es.max),clu)
		ct <- colnames(ss)[apply(ss,1,which.max)]
		ct <- data.frame(orict=rownames(ss),predct=ct)
	})
	f <- sub('.rds','',f)
	dir.create(paste0('/celltype/tabulasapiens/sctype/res/',f))
	saveRDS(rt,file=paste0('/celltype/tabulasapiens/sctype/res/',f,'/time.rds'))
	saveRDS(ct,file=paste0('/celltype/tabulasapiens/sctype/res/',f,'/res.rds'))
