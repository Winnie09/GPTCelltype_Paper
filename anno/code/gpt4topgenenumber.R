library(data.table)
d <- fread('/Users/zhichengji/Dropbox/research/gptcelltype/anno/res/gpt4aug3.csv',data.table=F)
d2 <- fread('/Users/zhichengji/Dropbox/research/gptcelltype/anno/res/sctype.csv',data.table=F)
d <- d[grep('_top',d[,1]),]
d$tissue <- sub('_',' ',d$tissue)
td <- paste0(sub('_top.*','',d$dataset),'_',d$tissue,'_',d$celltype)
...
write.csv(d,file='Dropbox/research/gptcelltype/anno/compiled/gpt4topgenenumber.csv',row.names=F)
